...
class MlpBarlowTwinsActor(nn.Module):
    ...
    def normalize(self,obs,obs_hist):
        ...
        hist_len = obs_hist.size(1)
        obs_hist = self.obs_normalizer(obs_hist.reshape(-1,self.num_prop)).reshape(-1,hist_len,self.num_prop)
        return obs,obs_hist
    ...
    def forward(self,obs,obs_hist):
        ...
        # obs_hist_full = obs_hist_full[:,self.num_hist:,:].view(b,-1)
        with torch.no_grad():
    ...
    def BarlowTwinsLoss(self,obs,obs_hist,priv,weight):
        ...
        # obs_hist = obs_hist[:,self.num_hist:,:].reshape(b,-1)
