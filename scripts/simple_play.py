...
from modules import *
from modules.onnx_export import export_policy_to_onnx, verify_onnx_model
...
def play(args):
    ...
    export_dir = os.path.join(ROOT_DIR, 'logs', train_cfg.runner.experiment_name, 'exported')
    onnx_path = os.path.join(export_dir, 'policy.onnx')
    print("\n" + "="*60)
    print("Exporting policy to ONNX format...")
    print("="*60)
    if export_policy_to_onnx(policy, onnx_path, device=env.device):
        # Verify the exported ONNX model
        print("\n" + "-"*60)
        print("Verifying ONNX model...")
        print("-"*60)
        verify_onnx_model(onnx_path, device=env.device)
    print("="*60 + "\n")
